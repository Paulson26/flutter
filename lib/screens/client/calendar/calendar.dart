...
class CalendarExample extends State<OnlineJsonData> {
  ...
  void dragEnd(AppointmentDragEndDetails appointmentDragEndDetails) async {
    final Meeting appointment =
        appointmentDragEndDetails.appointment as Meeting;
    ...
    var b = DateTime(
        ...
        draggingTime.minute,
        draggingTime.second,
        draggingTime.millisecond);

    var diff = b.difference(a).inDays;
    var diff2 = (b.difference(a).inSeconds * 1000);
    LocalDate c = LocalDate.dateTime(firstdate!);
    LocalDate d = LocalDate.dateTime(draggingTime);
    final difference = daysBetween(firstdate!, draggingTime);
    Period diffr = c.periodSince(d);
    ...
    String apptdate = DateFormat("yyyy-MM-dd").format(draggingTime);
    ...
    final response = await http.put(
      ...
      body: jsonEncode({
        "type": 'calendar',
        "data": data[0],
        "appt_id": appointment.id.toString(),
      }),
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.of(context).pushAndRemoveUntil(
          MaterialPageRoute(builder: (context) => const JsonData()),
          (route) => false);
      //Navigator.pushNamed(context, '/dashboard');
      toast("Appointment update requested!");
    } else {
      toast1("Error try again!");
    }
  }
...
class MeetingDataSource extends CalendarDataSource<Meeting> {
  ...
  Meeting? convertAppointmentToObject(
    ...
    return Meeting(
      ...
      recurrenceId: appointment.recurrenceId as int?,
